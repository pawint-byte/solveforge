  ...
  try {
    ...
    if (replitDomains) {
      ...
      try {
        ...
        console.log("Webhook setup skipped - will be configured on deployment");
      }
    } else {
      console.log("REPLIT_DOMAINS not available - webhook will be configured on deployment");
    }
...
// Initialize Stripe (wrapped in IIFE for top-level async)
initStripe().catch(console.error);

// CRITICAL: Register Stripe webhook route BEFORE express.json()
// This is required because the webhook needs raw Buffer, not parsed JSON
app.post(
  "/api/stripe/webhook",
